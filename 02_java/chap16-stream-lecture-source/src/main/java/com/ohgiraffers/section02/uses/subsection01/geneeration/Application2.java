...
/* 설명.
 *   기본 타입 스트림(Primitive Stream)
 *   - IntStream: int 타입 전용
 *   - LongStream: long 타입 정용
...
//        DoubleStream generate(Math::random)
        DoubleStream.generate(()-> {return Math.random();})     // 무한 스트립 생성
                .limit(5)               // 5개로 제한(필수!!)
                .forEach(num-> System.out.printf("%.3f",num));

        /* 설명. 4. 박싱(Boxing): 기본 타입을 패러 클래스로 변환(Stream에서는 제네릭이 걸린 Stream 또는 컬렉션 활용 시 박싱 유의) */
