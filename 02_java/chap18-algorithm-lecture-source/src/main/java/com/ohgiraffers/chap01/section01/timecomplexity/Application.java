...
//        Arrays.sort(arr);   // 기본 자료형은 퀵 정렬, 참조 자료형은 병합 정렬(nlongn)
...
//        System.out.println(Arrays.toString(reverse(arr)));
    ...
    * */
    /* 설명. 정렬된 배열과 찾고자 하는 타켓 값을 넣으면 타켓이 있는 위치를 반환하는 메소드 */
...
//        int right = arr.length - 1;
    ...
    /* 설명. 선행 시간 O(n) */
    /* 설명.
     *   입력값이 증가함에 따라 처리 시간이 선형적으로 증가하느 알고리즘이다.
    * */
    private static int[] reverse(int[] arr){
        int[] reversed = new int[arr.length];       // 같은 크기의 배열 생성
        for(int i = 0; i < arr.length; i++){
            reversed[i] = arr[arr.length - i - 1];  // 배열의 크기에 따라 반복 횟수가 증가되어진다.
        }
