...
//        System.out.println(Arrays.toString(arr));
        ...
        try(BufferedReader br = new BufferedReader(new InputStreamReader(System.in))){
            int length = Integer.parseInt(br.readLine());   // 가급적 한줄을 문자열로 읽어 숫자로 바꿀 대는
...
//            }
    ...
    /* 설명. 넘어온 데이터를 가지고 버블정렬 알고리즘 작성 시작 */
    public static void solution(int length, int[] arr){
        for (int i = 0; i < length; i++) {
            ...
            for(int j = 0; j < length-1; j++){
                if(arr[j] > arr[j+1]){          // 오름차순을 위해 오른쪽이 작ㅇ드면 swap을 위한 조건

                    /* 설명. 두 요소(j, j+1)의 위치를 swqmp */
                    int temp = arr[j];
                    arr[j] = arr[j+1];
                    arr[j+1] = temp;
                }
