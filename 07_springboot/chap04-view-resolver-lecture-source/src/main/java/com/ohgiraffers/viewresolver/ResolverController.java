...
public class ResolverController {
    ...
    * */
    ...
    public String stringReidrect(Model model){

        /* 설명. redirect 되어 요청이 다시 왔을 때 응답되는 view의 재료로 담는 걸 시도(model에 담긴 걸로는 전달 X) */
        model.addAttribute("message1","문자열로 뷰 이름 반환하며 리다이렉트");
        return "redirect:/";
    }

    @GetMapping("string-redirect-attr")
    public String stringReidrectFlashAttr(RedirectAttributes rttr){

        /* 설명. 스프링부트에서 리다이렉트 시에 값이 전달되게 하고 싶다면 RedirectAttributes에 Flash Attribute로 추가한다  */
        /* 설명. 추가로 내부적으로 HttpSession을 활용하기 때문에 Flash Attribute의 키 값이 기존 session의 키 값과ㅏ 중복되면 X */
        /* 설명. redirect 되어 요청이 다시 왔을 때 응답되는 view의 재료로 담는 걸 시도 */
        rttr.addFlashAttribute("message1","문자열로 뷰 이름 반환하며 리다이렉트");
        return "redirect:/";
    }
    /* 설명. forward 시에는 Model에 자료를, redirect 시에는 RedirectAttributes에 자료를 담는다. */

    @GetMapping("modelandview")
    public ModelAndView modelAndViewTest(ModelAndView mv){

        /* 설명. url 경로 상에 퀘리스링형태로 파리미터로 넘어간다. */
