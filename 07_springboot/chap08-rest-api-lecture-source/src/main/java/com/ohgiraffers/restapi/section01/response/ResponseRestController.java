...
public class ResponseRestController {
    @GetMapping("/hello")
//    @ResponseBody    // 핸들러 메소드에 응답이 json 형태로 되며 자바의 모든 타입 반환이 가능해 진다.
    ...
    /* 설명. Http 상태코드를 key로, 메시지를 value로 하는 Map현태로 반환 */
    @GetMapping("/map")
...
//    @GetMapping(value="/image/bytes/{filename}", produces = MediaType.IMAGE_PNG_VALUE)
    @GetMapping(value="/image/bytes/{filename}",
            produces = {MediaType.IMAGE_PNG_VALUE, MediaType.IMAGE_JPEG_VALUE}) // 없으면 파일이 안 나온다.
    public byte[] getImageWithBytes(@PathVariable String filename) throws IOException {
        Path path = Paths.get("C:\\Users\\Playdata\\OneDrive\\Desktop\\" + filename + ".png");
    ...
    * */
    ...
    public ResponseEntity<Resource> getImageWithResource(@PathVariable String filename)
            throws MalformedURLException {

        /* 설명. UrlResource: 파일 시스템 경로를 Resource 객체로 변환 */
        Path path = Paths.get("C:\\Users\\Playdata\\OneDrive\\Desktop\\" + filename);
         ...
         *    : inline은 브라우저에서 바로 열어서 표시, attachment는 무조건 다운로드 대화상자 표식
         *  3. filename 속성
