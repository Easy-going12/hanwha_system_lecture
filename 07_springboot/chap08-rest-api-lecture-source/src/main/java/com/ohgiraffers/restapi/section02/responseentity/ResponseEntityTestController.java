...
public class ResponseEntityTestController {
    ...
    /* 설명.
     *  ResponseEntity란?
     *   HttpRequest를 응답하게 위해 httpEntity를 상속받아 HttpStatus, HttpHeaders, HttpBody를
     *   정의하여 사용하는 클래스이다.
    ...
    * */
    ...
    public ResponseEntityTestController() {
        ...
        users.add(new UserDTO(2,"user02","pass02","홍길동",new java.util.Date()));
        users.add(new UserDTO(3,"user03","pass03","홍길동",new java.util.Date()));
    }
    ...
    /* 설명. 응답할 데이터가 따로 없다면 ResponseEntity의 제네릭이 ?를 활용할 수 있다.
            (feat. ResponseEntity 생성 시 build*/
    @PostMapping("/user")
    public ResponseEntity<?> registUser(@RequestBody UserDTO newMember){
        ...
        return ResponseEntity
                .created(URI.create("/entity/users/" + (lastUserNo + 1)))    // Response Header 중 "Location"에 감겨 돌아옴
                .build();
    }
