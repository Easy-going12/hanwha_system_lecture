...
public class EntityLifeCycleTest {
    ...
    public void 준영속성_detach_테스트(){
         ...
         *  detacch, clear, close가 준영속 상태를 만들기 위한 메소드이다.
         *
         * 설명.
         *  준영속을 사용하는 이유
         *   : JPA에서 엔티티를 더 이상 영속성 컨텍스트에서 관리하지 않음을 의미하며, 특정 상황에서 성능 최적화나
         *     특정 상황(flush를 특정시점에 별도로 사용)에서 성능 최적화나
         *     데이터 무결성 유지, 특정 작업 후 엔티티 변경 방자를 위해 사용 된다.
         *     (DB에서 가져온 객체들 DB에  영향을 크게 주지 않는 선에서 활용하고 싶을 때, 둘 다 활용도 가능)
         *
        * */
...
//        entityManager.clear();    // 영속성 컨텍스트에 있는 모든 영속 상태의 엔티티를 준영속으로 변경
        ...
        foundMenu1.setMenuPrice(9000);
        foundMenu2.setMenuPrice(9000);

        transaction.commit();

        Assertions.assertNotEquals(9000, entityManager.find(Menu.class, 11).getMenuPrice());
        Assertions.assertNotEquals(9000, entityManager.find(Menu.class, 12).getMenuPrice());
    }

    @Test
    // merge는 병합의 대상이 다를 경우 update가 되고 대상이 없을 경우 insert가 된다.
    public void 병합_merage_수정_테스트(){
        EntityTransaction transaction = entityManager.getTransaction();
        transaction.begin();
        ...
        Assertions.assertEquals("참치죽", managedMenu.getMenuName());
    }
}
