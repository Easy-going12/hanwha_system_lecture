...
public class BidirectionTest {
    ...
    public void 양방향_연관관계_매핑_조회_테스트(){
        ...
        System.out.println("10번 메뉴: " + foundMenu);         // Menu의 toString에서 Category와 관련된 필드를 주석
        ...
        /* 설명. 2. Category를 기준으로 */
        /* 설명. 부모 엔티티(OneToMany, FetchType.LAZY가 디폴트)는 해당 데이블만 조회한다. */
         ...
         *  1. 순환참조로 인한 stackOverflow를 발생키시므로 최소 한쪽 엔티티에서는 주석 처리를 해 주어야 한다.
         *  2. 아무리 Fetch.LAZY더라도 toString에서 연관관계의 필드가 있으면 toString()을 호출하며
         *        추가 select문을 발생시킨다,(N+1문제 발생)
         *  결론은, 왠만하면 연관관계를 가진 엔티티는 toString에서 연관관계와 관련된 필드는 생각하는 편이 좋다.
        * */
