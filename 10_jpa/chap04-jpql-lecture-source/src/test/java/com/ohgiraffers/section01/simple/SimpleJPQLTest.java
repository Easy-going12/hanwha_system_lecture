...
public class SimpleJPQLTest {
    ...
    public void TypedQuery를_이용한_단일행_단일열_조회_테스트(){
        String jpql = "SELECT m.menuName FROM menu_section01 m WHERE m WHERE m.menuCode = 7";

        /* 설명. 조회 결과 반환되는 값을 알고 있을 때는 TypedQuery를 활용하며 단일행인지 다중행인지 구분해서 사용 */
        ...
        Assertions.assertEquals("A",resultMenuName);
    }
...
//        Assertions.assertEquals("민트미역국", resultMenuName);
        ...
        /* 설명. 일부 속성만 추출하는 Projectiion의 하나의 경우만 테스트 */
        ...
        Query query = entityManager.createQuery(jpql);
        List<Object[]> resultMenu = query.getResultList();

        Object[] resultRow = resultMenu.get(0);
