...
public class UserServiceImpl implements UserService {
    ...
    public void registUser(UserDTO userDTO) {
        ...
        /* 설명. UserDTO로 넘어온 사용자의 암호(평문)를 BCrypt 암호화 해서(디이제스트) UserEntity에 전달 */
    ...
    public UserDetails loadUserByUsername(String email) throws UsernameNotFoundException {
        ...
        /* 설명. 넘어온 email과 일치하는 email을 가진 회원을 조회해서 UserEntity로 변혼(조회) 받음 */
        UserEntity loginUser = userRepository.findByEmail(email);       // 쿼리 메소드를 활용한 email where절  활용

        /* 설명. 사용자가 로그인 시 아이디(이메일)를 잘못 입력 햇다면 */
        if(loginUser == null){
