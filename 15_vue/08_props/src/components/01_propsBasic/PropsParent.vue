<!--
    ...
    Slot는 부모가 자식에게 태그를 전다할 때 사용

    Prop는 기본적으로 readonly의 개념을 통해 지식 컴포넌트에서
    수정해도 부모에 영향을 주지 않다(안티패턴 방지)

    SLot은 이름(named)을 활용해 자식에게 선택권을 주거나
    런타임 시점에 동작으로 이름을 바꿔서 전달할 수 있다.
-->

<template>
    <div :class="{container:true, dark:darkState}">
        
        <!-- 원하는 이름으로 부모 컴포넌트의 데이터(상태)를 전달할 수 있다.(Props, 일반적으로는 같은 이름을 사용) -->
        <PropsChild :message="message2"/> <!-- message2에 값을 message 변수 값에 넣는다 -->
